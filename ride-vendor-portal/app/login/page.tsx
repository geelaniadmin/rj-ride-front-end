...
import { useSessionStore, useVendorStore } from "@ride/shared";
    ...
    <div className="min-h-screen flex items-center justify-center bg-page-bg">
      <div className="bg-card-bg rounded-xl shadow-lg border border-border p-10 w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-text-primary">RIDE Vendor Portal</h1>
          <p className="text-sm text-text-muted mt-1">Sign in to manage your fleet and trips</p>
        </div>

        <div className="space-y-3">
          {activeVendors.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-8">
              No vendors registered yet. Create a vendor in the admin portal to get started.
            </p>
          ) : (
        ...
        <p className="text-xs text-text-muted text-center mt-8">
          New vendors created in the admin portal appear here automatically
        </p>
      </div>
    </div>
  );
